...
export async function getHouses(page: number = 1, perPage: number = 10): Promise<GetHouses> {
    const url = new URL(appConfig.HOMES_API_URL);
    ...
    return axios.get<GetHouses>(url.toString(), { headers: { Accept: 'application/json' } }).then(({ data, status }) => {
        ...
        if (reason.response?.status && reason.response?.status >= 500 && reason.response?.status <= 599) {
